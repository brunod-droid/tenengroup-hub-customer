...
export default function VIPPolicyFullSource() {
  ...
  return (
    <main style={pageStyle}><div style={shell}>
      <Header type="VIP / SPECIAL CUSTOMERS" title="Special Customer / VIP Policy" color="#a855f7" sourceHref="/docs/policies/vip-policy.pdf" sourceLabel="Open original PDF file" subtitle="Special customers should receive priority service and enhanced compensation when the standard policy is not enough." />
      <Section title="Core Principle" color="#a855f7"><p style={p}>Make the customer feel recognized. The reply should clearly say they are a VIP/special customer and that we are offering better compensation as an exception.</p><BulletList items={["Give priority service.", "Offer higher compensation straight away when relevant.", "Use the special policy when the customer is upset about the normal policy.", "Emphasize that this level of compensation is not usually offered."]} /></Section>
      <Section title="Categories & Compensation" color="#7c3aed"><div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 14 }}>{categories.map((cat) => <div key={cat.name} style={{ ...card, marginTop: 0 }}><h3>{cat.name}</h3><p style={p}><b>Who:</b> {cat.who}</p><p style={p}><b>Tag:</b> {cat.tag}</p><BulletList items={cat.items} /></div>)}</div></Section>
      <Section title="Messaging Guidance" color="#ec4899"><p style={p}>Create a special feel. Explain that we usually do not give this compensation, but we want to do more because this is a VIP/special customer.</p><div style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 14, padding: 16, marginTop: 12 }}><p style={p}><b>Example tone:</b></p><p style={p}>“As one of our valued VIP customers, your satisfaction is extremely important to us. While this is not our usual policy, we would like to make an exception and offer enhanced compensation as a special gesture of appreciation.”</p></div></Section>
      <Section title="Additional Policy Note" color="#0f766e"><p style={p}>Shipping refund can still be given as compensation, especially if the shipping fee is higher than the 10% refund.</p></Section>
    </div></main>
  );
}
