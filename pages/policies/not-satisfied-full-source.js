  ...
  return (
    <main style={pageStyle}>
      <div style={shell}>
        ...
        <div style={{ ...card, borderTop: "8px solid #f59e0b" }}>
          <span style={badge}>FULL SOURCE</span><span style={badge}>ITEM RECEIVED</span>
          <h1 style={{ fontSize: 42, margin: "8px 0 6px", fontWeight: 950 }}>Not Satisfied Policy</h1>
          <p style={p}>Use when the jewelry was produced correctly but the customer is not satisfied with design, font, thickness, quality, or similar preference-based issues.</p>
        </div>
        ...
        <Section title="Exchange Procedure by Region" color="#b45309">
          <h3>US / Canada</h3><BulletList items={["Check the link/item ID from the customer or request all details for the new item.", "Escalate to the Shipping Coordinator to send return label and instructions.", "Only once the item is returned, premium and non-premium exchanges can proceed."]} />
          <h3 style={{ marginTop: 18 }}>UK</h3><BulletList items={["Check the link/item ID or request all details for the new item.", "Send return instructions with return label link.", "Escalate according to customer request.", "Only once the item is returned, exchange can proceed."]} />
          <h3 style={{ marginTop: 18 }}>Non US / UK Customers</h3><BulletList items={["Customer returns the item at their own expense.", "Request link/item ID or all needed details for the new item.", "No shipping proof is required.", "Once customer informs us the item was returned, proceed according to item type and policy."]} />
        </Section>
